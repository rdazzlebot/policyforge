...
| **ONC certification criteria** | US federal regulation (45 CFR 170.315) — public domain                                                                            | Bundled directly in `data/frameworks/cfr-170-315-onc-certification/`, sourced from eCFR's public API via `policyforge etl-onc`. One criterion is one control, cited as `170.315(g)(10)`. Reserved criteria are excluded and reported by id each run — `170.315(i)` is reserved entirely, and `(j)(1)`–`(j)(19)` are a reserved range                                                                                                                                          |
