...
| `litellm`       | Whatever LiteLLM routes to — commonly OpenRouter, which may itself route to further providers | third-party                 | **Two hops.** OpenRouter's terms *and* the terms of whichever upstream serves your model. This is the least transparent option |
...
| **Catalog fetches** (`etl-oscal`, `etl-hipaa`, `etl-hipaa-crosswalk`) | NIST (`csrc.nist.gov`, `raw.githubusercontent.com`) and eCFR (`www.ecfr.gov`) | Explicit command only                  | Nothing — these are **outbound requests for public government works.** No content of yours is sent |
