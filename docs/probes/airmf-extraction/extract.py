...
def main() -> int:
    html = (
        ...
        .with_name("source-airc.html")
        .read_text(encoding="utf-8", errors="replace")
    )
    rows = [{"id": m.group("id"), "text": clean(m.group("text"))} for m in ROW.finditer(html)]
    ...
    for fn in ("Govern", "Map", "Measure", "Manage"):
        nums = sorted(int(c["id"].split()[1]) for c in cats if c["id"].startswith(fn))
    ...
    print(
        "  per function:",
        {
            fn: sum(1 for c in cats if c["id"].startswith(fn))
            for fn in ("Govern", "Map", "Measure", "Manage")
        },
    )
    return 0
