...
`170.315(i)` is `[Reserved]` and is **not emitted**. A reserved criterion
emitted as a control would be permanently unsatisfiable — it counts, it
renders, it crosswalks, and it means nothing — and every coverage report
after it would carry a gap that can never close. `tests/test_onc_loader.py`
asserts the source's letter run is unbroken *and* that `170.315(i)` is
absent, so the omission is asserted rather than silent. That pairing is
policyforge-80's, following `test_reserved_sections_are_absent` in the
information-blocking loader.
"""
...
def is_reserved(paragraph: str) -> bool:
    ...
    **A reserved criterion is found and then not emitted.** `170.315(i)` is
    ...
    reason for the pairing in `tests/test_onc_loader.py`: the letter run
    **in the source** is asserted unbroken from `(a)` to `(j)`, *and* the emitted
    catalog is asserted not to contain `170.315(i)`. Together they prove the
    parser saw ten and emitted nine deliberately.
